...
class EnhancedSQLValidator:
    def __init__(self, base_url: str, api_key: str, agent_id: str, dataset_ids: List[str],
                 excel_path: str, max_workers: int = 1, session_id: str = None):
        """
        增强版SQL验证器 - 宽松匹配标准
        ...
        self.session_id = session_id  # 可以为None，在需要时创建
        ...
        self.session_api_url = f"{self.base_url}/api/v1/agents/{agent_id}/sessions"

        # 使用Session保持连接复用
        self.session_client = requests.Session()
        self.session_client.headers.update({
    ...
    def create_session(self) -> str:
        ...
        """
        try:
            logger.info("正在创建新的session...")
            response = self.session_client.post(self.session_api_url, json={})
            ...
            if response.status_code == 200:
                ...
                if data.get("code") == 0 and "data" in data:
                    ...
                    if session_id:
                        logger.info(f"成功创建session: {session_id}")
                        self.session_id = session_id
                        return session_id
                    else:
                        logger.error("API返回成功但未找到session_id")
                else:
                    logger.error(f"API返回错误: {data.get('message', '未知错误')}")
            else:
                logger.error(f"创建session请求失败，状态码: {response.status_code}")

        except Exception as e:
            logger.error(f"创建session时发生错误: {e}")

        # 如果创建失败，使用默认session_id
        default_session = "default_session_" + datetime.now().strftime("%Y%m%d%H%M%S")
        logger.warning(f"使用默认session_id: {default_session}")
        self.session_id = default_session
        return default_session

    # ==================== 流式API调用方法 ====================

    def query_agent_for_sql_stream(self, question: str) -> str:
        """调用智能体接口获取生成的SQL - 流式版本"""
        # 如果session_id不存在，则创建一个
        if not self.session_id:
            self.create_session()

        # 修改请求体格式，增加session_id字段和stream=true
        payload = {
            "question": question,
            "stream": True,  # 启用流式
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"正在查询智能体(流式) - 尝试 {attempt + 1}/{max_retries}")
                        ...
                        try:
                            ...
                            if line_str.startswith('data:'):
                                if line_str.startswith('data: '):
                                    ...
                                    line_str = line_str[5:]  # 移除 "data:" 前缀

                            # 解析JSON
                            event_data = json.loads(line_str)
                            logger.debug(f"收到事件: {event_data.get('event')}")

                            # 处理消息事件
                            if event_data.get('event') == 'message' and 'data' in event_data:
                            ...
                            logger.warning(f"JSON解析错误: {e}, 行内容: {line_str}")
                        except Exception as e:
                            logger.warning(f"处理流式数据时出错: {e}")

                logger.info(f"流式响应完整内容: {full_content}")
                return self.extract_complete_sql(full_content)
    ...
    def query_agent_for_sql(self, question: str) -> str:
        """调用智能体接口获取生成的SQL - 兼容性包装器，使用流式版本"""
        return self.query_agent_for_sql_stream(question)
    ...
    def process_single_test_case(self, index: int, row: pd.Series, total_rows: int) -> List[Dict]:
        ...
        # 步骤1: 调用智能体获取生成的SQL（使用流式版本）
        start_time = time.time()
        generated_sql = self.query_agent_for_sql_stream(question)  # 使用流式版本
        ...
        for dataset_id in self.dataset_ids:
            ...
            if dataset_id != self.dataset_ids[-1]:  # 如果不是最后一个知识库
                logger.info("知识库间间隔1秒...")
                time.sleep(1)
    ...
    def _combine_simplified_results(self, index: int, question: str, expected_sql: str, generated_sql: str,
        ...
        result = {
            ...
            '预期SQL': expected_sql,
            '生成SQL': generated_sql,
            '会话ID': self.session_id,  # 添加session_id到结果中
    ...
    def _create_error_result(self, index: int, question: str, expected_sql: str,
        ...
        for dataset_id in self.dataset_ids:
            result = {
                ...
                '预期SQL': expected_sql,
                '生成SQL': '',
                '会话ID': self.session_id,  # 添加session_id到错误结果中
                'SQL验证结果': '验证失败',
                '是否完全匹配': False,
                '相似度': 0,
                '响应时间秒': response_time,
    ...
    def run_comprehensive_validation(self, use_parallel: bool = True) -> pd.DataFrame:
        ...
        logger.info(f"会话ID: {self.session_id}")  # 添加session_id显示
    ...
    def generate_simplified_report(self, results_df: pd.DataFrame):
        ...
        print(f"会话ID: {self.session_id}")  # 显示session_id
        print(f"测试文件: {os.path.basename(self.excel_path)}")  # 显示测试文件名
    ...
    def save_simplified_report(self, results_df: pd.DataFrame) -> str:
        ...
        output_dir = "宽松匹配_SQL验证报告"
        os.makedirs(output_dir, exist_ok=True)

        # 使用文件名和session_id作为报告文件名的一部分
        file_basename = os.path.basename(self.excel_path).replace('.xlsx', '')
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = os.path.join(output_dir,
                                   f"宽松匹配_SQL验证报告_{file_basename}_{self.session_id}_{timestamp}.xlsx")

        try:
            with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
                # 重新排列列的顺序，将关键信息放在前面
                base_columns = [
                    '问题序号', '问题内容', '知识库ID', '会话ID', 'SQL验证结果',
                    '是否完全匹配', '相似度', '响应时间秒',
                    '表名匹配', '字段包含', '条件兼容', '分组兼容',
                    '表名匹配率', '字段匹配率',
                    '缺失表名', '缺失字段', '额外表名', '额外字段',
    ...
    def _create_simplified_summary_sheet(self, writer, results_df: pd.DataFrame):
        ...
        summary_data.append({
            '统计指标': '测试文件',
            '数值': os.path.basename(self.excel_path),
            '百分比': '-'
        })
...
def process_single_excel_file(excel_file: str, base_url: str, api_key: str, agent_id: str,
    ...
    validator = EnhancedSQLValidator(
        ...
        max_workers=8,
        session_id=None  # 设置为None，让验证器自动创建session_id
    )

    # 创建session_id
    session_id = validator.create_session()
    logger.info(f"为文件 {os.path.basename(excel_file)} 创建session: {session_id}")

    # 运行综合验证
    start_time = time.time()
    results_df = validator.run_comprehensive_validation(use_parallel=False)
    end_time = time.time()

    logger.info(f"文件 {os.path.basename(excel_file)} 处理完成，耗时: {end_time - start_time:.2f} 秒")
    ...
    if not results_df.empty:
        ...
        logger.info(f"📊 文件 {os.path.basename(excel_file)} 最终统计:")
        ...
        logger.error(f"文件 {os.path.basename(excel_file)} 验证失败，无结果生成")
        validator.clear_cache()
        return ""


def generate_summary_report(report_files: List[str], output_dir: str = "宽松匹配_SQL验证报告") -> str:
    """
    生成总体验证报告

    Args:
        report_files: 所有报告文件路径列表
        output_dir: 输出目录
    ...
    for report_file in report_files:
        try:
            # 读取单个报告文件
            df = pd.read_excel(report_file, sheet_name='验证结果')
            file_name = os.path.basename(report_file)

            # 提取session_id从文件名中
            session_id = "未知"
            if '会话ID' in df.columns and len(df) > 0:
                session_id = df['会话ID'].iloc[0]

            # 计算统计信息
            total_tests = len(df)
            exact_matches = len(df[df['是否完全匹配'] == True])
            exact_match_rate = (exact_matches / total_tests) * 100
            avg_similarity = df['相似度'].mean() * 100
            avg_response_time = df['响应时间秒'].mean()

            summary_data.append({
                '文件名': os.path.basename(report_file).replace('宽松匹配_SQL验证报告_', '').replace('.xlsx', ''),
                '会话ID': session_id,
                '总测试数': total_tests,
                '完全匹配数': exact_matches,
                '匹配率%': round(exact_match_rate, 2),
                '平均相似度%': round(avg_similarity, 2),
                '平均响应时间(秒)': round(avg_response_time, 2),
                '表名匹配率%': round(df['表名匹配率'].mean() * 100, 2),
                '字段匹配率%': round(df['字段匹配率'].mean() * 100, 2)
            })
    ...
    # 保存汇总报告
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    summary_file = os.path.join(output_dir, f"总体验证报告汇总_{timestamp}.xlsx")
    ...
    try:
        ...
        print("📊 总体验证报告汇总")
        print(f"{'=' * 80}")
        print(f"总文件数: {len(summary_data)}")
        print(f"总测试次数: {summary_df['总测试数'].sum()}")
        print(f"总体匹配率: {summary_df['匹配率%'].mean():.1f}%")
        print(f"总体平均相似度: {summary_df['平均相似度%'].mean():.1f}%")
...
if __name__ == "__main__":
    # 配置参数
    BASE_URL = "http://172.17.2.65:80"
    API_KEY = "ragflow--UDyZmqBf9OAtu5LJCGE3mYWeBEfGH1ee5czJHW2bfw"
    AGENT_ID = "9e4e7a5cd01011f0aff70242c0a85006"
    DATASET_IDS = ["a0c2dfacca6211f0be540242c0a85006"]
    TEST_DIR = "test"  # 测试文件目录

    logger.info("开始执行多文件宽松匹配SQL验证...")
    ...
    if not excel_files:
        logger.warning(f"在目录 {TEST_DIR} 中未找到Excel文件")
        exit(1)

    logger.info(f"找到 {len(excel_files)} 个Excel文件: {[os.path.basename(f) for f in excel_files]}")

    # 处理每个Excel文件
    report_files = []
    for excel_file in excel_files:
        try:
            report_file = process_single_excel_file(
            ...
            logger.error(f"处理文件 {excel_file} 时发生错误: {e}")
            continue

    # 生成汇总报告
    if report_files:
        summary_report = generate_summary_report(report_files)
        if summary_report:
            logger.info(f"所有文件处理完成！汇总报告: {summary_report}")
        else:
            logger.info("所有文件处理完成！但汇总报告生成失败")
    else:
        logger.error("没有成功生成任何报告文件")